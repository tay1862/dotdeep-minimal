        ...
        <nav className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="lg:hidden w-10 h-10 flex items-center justify-center rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
        ...
        <div className="bg-surface/95 backdrop-blur-2xl border-t border-border-default">
          <div className="container py-5">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.key}>
                    <Link
                      href={`/${locale}${item.href}`}
                      className={`flex items-center justify-between px-4 py-3 rounded-xl text-base font-medium transition-colors ${active ? 'text-brand-500 bg-brand-50 dark:bg-brand-950/50' : 'text-on-surface hover:bg-neutral-100 dark:hover:bg-neutral-800/60'}`}
            ...
            <div className="mt-4 pt-4 border-t border-border-default">
              <Link
                href={`/${locale}/contact`}
                className="flex items-center justify-center w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold py-3 rounded-xl transition-colors"
                onClick={() => setMobileOpen(false)}
              >
                {t('contact')}
              </Link>
